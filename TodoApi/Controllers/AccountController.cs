        ...
        public async Task<IActionResult> SignUp (SignUpDto signUpDto)
        {
            if (ModelState.IsValid)
            {
                ...
                if(result.Succeeded)
                {
                    ...
                    await _auditService.LogActionAsync(user.Id, "SignUp", $"{user.UserName} has Signed Up", ip);
            ...
            if (ModelState.IsValid)
            {
                ...
                if (user != null)
                {
                    ...
                    if (result)
                    {
                        ...
                        var ip = HttpContext.Connection.RemoteIpAddress.ToString();
                        await _auditService.LogActionAsync(user.Id, "Login", $"{user.UserName} has Logged In", ip);
